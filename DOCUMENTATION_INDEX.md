...
| Artifact | Purpose |
|----------|---------|
| [`docs/COMMUNITY_CORPUS.md`](docs/COMMUNITY_CORPUS.md) | Contract — what consensus does and does **not** prove, the surface digest, corpus layout, `attesters.json`, coordinate grammar, verdict/exit table, phase-2 sandbox contract |
| [`src/mcp_warden/corpus.py`](src/mcp_warden/corpus.py) | Corpus fetch (path or git URL @ exact sha), attester loading, per-entry signature + self-consistency verification, `WRD-CONSENSUS-*` verdicts |
...
| [`src/mcp_warden/cli_corpus.py`](src/mcp_warden/cli_corpus.py) | `check` glue: coordinate preflight before spawn, consensus adjudication after drift |
...
| [`tests/test_corpus.py`](tests/test_corpus.py) / [`tests/test_corpus_cli.py`](tests/test_corpus_cli.py) | Coordinate table, every verdict, every fail-closed path (unknown attester, missing/corrupt/wrong sidecar, tampered entries, unreachable/wrong-ref corpus, URL clone at exact sha), SARIF/JSONL, default `check` unchanged |
